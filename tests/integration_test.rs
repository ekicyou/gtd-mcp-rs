//! Integration tests for GtdServerHandler (MCP server handler)
//!
//! These tests verify the end-to-end behavior of the MCP server handler,
//! including all tool implementations like inbox, list, update, change_status, and empty_trash.
...
use mcp_attr::{bail_public, Result as McpResult};
...
fn test_normalize_task_id() {
    ...
    assert_eq!(gtd_mcp::validation::normalize_task_id("task-1"), "task-1");
    assert_eq!(
        gtd_mcp::validation::normalize_task_id("meeting-prep"),
        "meeting-prep"
    );
    assert_eq!(gtd_mcp::validation::normalize_task_id("call-sarah"), "call-sarah");
    assert_eq!(gtd_mcp::validation::normalize_task_id("  task-1  "), "task-1");
    assert_eq!(
        gtd_mcp::validation::normalize_task_id("  meeting-prep  "),
        "meeting-prep"
    ...
    async fn test_change_task_status_unified_api() {
        ...
        let result = handler
            ...
            .await;
        assert!(result.is_ok());
        let task_id = common::extract_id_from_response(&result.unwrap());
    ...
    async fn test_batch_change_status_with_id_normalization() {
        ...
        let task_id1 = common::extract_id_from_response(&result1.unwrap());
        ...
        let task_id2 = common::extract_id_from_response(&result2.unwrap());
    ...
    async fn test_update_task_project_and_context() {
        ...
        let project_id = common::extract_id_from_response(&project_result.unwrap());
    ...
    async fn test_update_project_name() {
        ...
        let result = handler
            ...
            .await;
        assert!(result.is_ok());
        let project_id = common::extract_id_from_response(&result.unwrap());
    ...
    async fn test_trash_task_workflow_comparison() {
        ...
        let result = handler
            ...
            .await;
        assert!(result.is_ok());
        let task_id_1 = common::extract_id_from_response(&result.unwrap());
        ...
        let result = handler
            ...
            .await;
        assert!(result.is_ok());
        let task_id_2 = common::extract_id_from_response(&result.unwrap());
    ...
    async fn test_trash_notas_from_different_statuses() {
        ...
        let result = handler
            ...
            .await;
        assert!(result.is_ok());
        let inbox_task_id = common::extract_id_from_response(&result.unwrap());
        ...
        let result = handler
            ...
            .await;
        assert!(result.is_ok());
        let next_action_task_id = common::extract_id_from_response(&result.unwrap());
        handler
            .change_status(
        ...
        let result = handler
            ...
            .await;
        assert!(result.is_ok());
        let done_task_id = common::extract_id_from_response(&result.unwrap());
        handler
    ...
    async fn test_delete_context_after_removing_task_reference() {
        ...
        let task_id = common::extract_id_from_response(&response);
    ...
    */
    ...
    async fn test_calendar_tasks_partial_failure() {
        ...
        let result = handler
            ...
            .await;
        assert!(result.is_ok());
        task_ids.push(common::extract_id_from_response(&result.unwrap()));
